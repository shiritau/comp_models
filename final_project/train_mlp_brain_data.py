...
class MLP:
    ...
    def forward(self, P):
        assert len(P[0]) == self.input_size
        self.h1 = []
        self.s1 = []
        self.h2 = []
        self.s2 = []
        ...
        for i in range(self.hidden_size):
            ...
            for j in range(self.input_size):
                self.W0[i][j] = self.W0[i][j] - self.lr * loss * ((1 - self.tanh(self.h3) ** 2) * self.W1[0][i]) * (
                        1 - self.tanh(self.h1[i]) ** 2) * data[0][j]
...
if __name__ == '__main__':
    ...
    mlp_model = MLP(hidden_size=10, input_size=240*240, output_size=1, lr=0.0001)
    current_time = datetime.now().strftime("%d_%m_%Y-%H_%M_%S")
    task = Task.init(project_name="tumour_detection", task_name=f"mlp_brain_{current_time}")

    train(DF_PATH, mlp_model, NUM_EPOCHS, WEIGHTS_DIR, task.name, ACCURACY_THRESHOLD)
