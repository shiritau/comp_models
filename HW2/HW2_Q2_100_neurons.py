...
def backprop(y_true, y_pred, P, W0, W1, N_middle, s1, h1, h2, b, lr=0.001):
    ...
    for i in range( N_middle):
        W1[0][i] = W1[0][i] - lr* et_a * da_dw1[i]
    for i in range(100):
        W0[i][0] = W0[i][0] - lr*et_a * da_ds1[i] * ds1_dw0[i][0]
        W0[i][1] = W0[i][1] - lr*et_a * da_ds1[i] * ds1_dw0[i][1]
        W0[i][2] = W0[i][2] - lr*et_a * da_ds1[i] * ds1_dw0[i][2]
...
if __name__ == '__main__':
    ...
    for i in range(100):
        ...
        W0, W1, loss = train(W0, W1, N_MIDDLE, num_iter=1000000, density=density)
        if loss[-1]<best_loss:
            best_loss=loss[-1]
            best_loss_list = loss
            best_W0=W0.copy()
